"""Stage policy: a short wheat bridge plus pastures built before day 3.

Four wheat plots keep feed from consuming the animal budget. Eight pastures
are up before the first shop, so day 3 places animals instead of building.
Product is held above base. Two units may be sold only when cash cannot
buy the next day's wheat.
"""
...
def _focus(demand: Mapping[str, float], day: int, owned: int) -> str:
    ...
    if best == "GOOSE" and day < 21 and max(scores["COW"], scores["SHEEP"]) <= 0:
        return str(_LOCK["animal"] or "COW")
    locked = _LOCK["animal"]
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    # Sell a little only to afford wheat. A standing herd that cannot buy
    # feed escapes; dumping the book to buy more animals does not.
    need_cash = live > 0 and money < 180 and wheat_have < live * 2
