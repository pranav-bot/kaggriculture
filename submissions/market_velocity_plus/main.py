"""Market Velocity Plus: adaptive crops, controlled labor, no livestock diversion."""

import os
import sys
from typing import Any, Dict

if "__file__" in globals():
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
else:
    sys.path.insert(0, os.getcwd())

from kaggriculture import ActionController, Actions, CROPS, Plants


class MarketVelocityPlusController(ActionController):
    def __init__(self):
        super().__init__(
            target_crop=Plants.MELON,
            auto_water=True,
            auto_harvest=True,
            auto_fertilize=True,
            auto_feed_animals=False,
            auto_care_animals=False,
            auto_collect_fertilizer=True,
            auto_dig_weeds=True,
            auto_sell=True,
            auto_expand_land=True,
            auto_hire_hands=True,
            max_hires_per_day=4,
            min_sell_margin=0.15,
        )

    def act(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        day = int(obs.get("day", 0))
        if day < 26:
            prices = obs.get("market", {}).get("prices", {})
            shops = obs.get("town", {}).get("unlocked_shops", [])
            demand = Actions.calculate_town_daily_consumption(shops)
            remaining = 30 - day
            scores = {}
            for name, crop in CROPS.items():
                if crop.time_to_first_yield <= remaining:
                    price = float(prices.get(name, crop.base_market_price))
                    demand_bonus = 1.0 + 0.08 * float(demand.get(name, 0))
                    scores[name] = price * crop.yield_per_tile_per_day * demand_bonus - crop.seed_cost
            if scores:
                self.target_crop = max(scores, key=scores.get)
        self.auto_expand_land = day < 26
        self.auto_hire_hands = day < 28
        return super().act(obs)


controller = MarketVelocityPlusController()


def agent(obs: Dict[str, Any]) -> Dict[str, Any]:
    return controller.act(obs)
