"""Learned milk floor plus catalog rules for every other good.

Fertilizer has no sink: collect always, sell down to $40. Wool and melon
use a log scarcity / square glut, so they never dump. Eggs wait for a
hinge the herd cannot cross, so they hold until the last days. Strawberries
follow the milk hold. Labor is care_mill. Milk uses the table in
q_values.json.
"""
...
def _sell_qty(
    ...
    if item != "MILK":
        # Wool and melon: a dump walks a square glut. Hold until the last
        # days unless the shed is already tight.
        if item in ("WOOL", "MELON"):
            if day < 27 and shed_total < 85 and have < 24:
                return 0
            return min(have, 2)
        # Eggs stay near $50 until the deficit crosses T=332. An 18-goose
        # herd cannot do that. Hold, then a trickle so the hinge is not spent.
        if item == "EGG":
            if price < 80 and day < 28:
                return 0
            return min(have, 2)
        if item == "STRAWBERRY":
            if day < 27 and shed_total < 78 and have < 20:
                return 0
            return min(have, 3)
        if item in ("TOMATO", "CARROT"):
            if day < 27 and shed_total < 85:
                return 0
            return min(have, 2)
