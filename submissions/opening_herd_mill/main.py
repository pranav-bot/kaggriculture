"""opening_herd_mill — solvent incumbent-cow opening with bounded cash conversion.

This variant preserves compound_implementation's feed, hire, and animal order
priorities, then converts inventory earlier at measured shed-pressure levels
and liquidates the terminal stock in merged product orders.  It only changes
the collapse point where demand-gated selection left the farm without an
incumbent herd: a small cow opening is permitted under the existing solvency
and feed safeguards, and secondary species wait for a productive cow base.
"""
...
MELON_SEEDS = 2
MELON_SEED_COST = 80
INCUMBENT_COW_OPENING = 2
INCUMBENT_COW_GATE = 4
SELL_START_DAY = 14
...
def _cow_status(scan: Mapping[str, Any], private: Mapping[str, Any]) -> Tuple[int, int]:
    """Return (owned, productive) cows without treating shed stock as productive."""
    shed = private.get("shed", {})
    carried = sum(int((inv or {}).get("COW", 0)) for inv in private.get("inventories", []))
    owned = int(scan["counts"].get("COW", 0)) + int(shed.get("COW", 0)) + carried
    productive = sum(
        1
        for kind, _pos, tile in scan["rows"]
        if kind == "animal"
        and str(tile.get("animal")) == "COW"
        and int(tile.get("yield_units", 0)) > 0
    )
    return owned, productive
...
def _focus(
    ...
    if int(day) == 0:
        _LOCK["animal"] = None
    cow_owned, productive_cows = _cow_status(scan, private)
    if cow_owned < INCUMBENT_COW_GATE or productive_cows < INCUMBENT_COW_GATE:
        _LOCK["animal"] = "COW"
        return "COW"
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    # Before demand is useful, establish only a small incumbent cow base.
    # Keep the normal demand gate for every later species.
    cow_owned, _productive_cows = _cow_status(scan, private)
    incumbent_opening = animal == "COW" and cow_owned < INCUMBENT_COW_GATE
    demand_backed = _scores(demand).get(animal, 0) > 0
    if (
        (incumbent_opening or demand_backed)
        and owned < max(target, INCUMBENT_COW_OPENING if incumbent_opening else target)
        and len(orders) < 9
    ):
        free = len(scan["empty"][spec["structure"]])
        # Buy as many as we can afford, keeping operating reserve
        next_hire_reserve = Actions.hire_cost(hires) if hires < MAX_HANDS else 0
        affordable = int(
            (money - OPERATING_RESERVE - next_hire_reserve - wheat_floor * wheat_price)
            // spec["cost"]
        )
        affordable = max(0, affordable)
        buy_target = max(target, INCUMBENT_COW_OPENING if incumbent_opening else target)
        buy = min(affordable, free, buy_target - owned, 3)  # Max 3 per turn to leave order slots
...
def agent(obs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    animal = _focus(demand, day, owned_any, scan, obs.get("private", {}))
