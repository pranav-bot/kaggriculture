"""Cow-only probe. Wheat plots, sell rule, and herd ramp come from the environment.

STAGE_WHEAT=0|4|8  STAGE_SELL=hold|trickle  STAGE_RAMP=slow|fast
Species is fixed so those three stages can be compared on their own.
"""
...
def _cfg() -> Dict[str, Any]:
    wheat = int(os.environ.get("STAGE_WHEAT", "8"))
    sell = os.environ.get("STAGE_SELL", "hold")
    ramp = os.environ.get("STAGE_RAMP", "slow")
    cap = os.environ.get("STAGE_CAP", "18")
    return {
        "wheat": wheat,
        "sell": sell if sell in ("hold", "trickle") else "hold",
        "ramp": ramp,
        "cap": cap,
    }
Pos = Tuple[int, int]
...
def _sell_qty(
    ...
    save_land: bool, need_cash: bool,
) -> int:
    """Hold premium goods while the town is still drawing the book down.

    The shed holds 100. Selling a little every turn spends the scarcity
    premium. Sell only to free space, or on the last days, when the
    accumulated deficit is what the price is paying for.
    """
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    cap_raw = str(_cfg()["cap"])
    if cap_raw == "drain":
        cap_n = _drain_cap(animal, demand, len(quadrants))
    elif cap_raw == "match":
        spec_cap = SPECIES[animal]
        shop = max(0.0, float(demand.get(spec_cap["product"], 0.0)) - 1.0)
        cap_n = 0 if shop <= 0 else max(4, int(round(shop / spec_cap["per_day"])))
    else:
        cap_n = int(cap_raw)
    target = min(_target(day, len(quadrants)), cap_n, max(0, 18 - others))
    owned = focus_owned
    ...
    plots = int(_cfg()["wheat"])
    if day == 0 and plots > 0 and growing < plots and len(orders) < 10:
        qty = min(plots - int(seeds.get("WHEAT", 0)), int(money // 10))
        if qty > 0:
...
def _units(
    ...
    for index, position in enumerate(positions):
        ...
        # Build the full set before any animals, on every cap. Empty-tile
        # counts then stay aligned and the shop draw can be compared.
        need_struct = 18
        ...
        plots = int(_cfg()["wheat"])
        if plots > 0 and day < 2 and int(seeds.get("WHEAT", 0)) - planted > 0 and scan["crops"].get("WHEAT", 0) + planted < plots:
            target = take_nearest(pos, list(scan["empties"]))
            if target is not None:
                planted += 1
