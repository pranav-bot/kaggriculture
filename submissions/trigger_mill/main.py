"""Quant milk schedule, with a price trigger for a shared book.

Solo towns keep the scipy path: hold milk through day 16, drip about 13%
of the shed per day after that, and clear from day 28. If the quoted milk
price falls under 1.15× base, another seller is spending the deficit, so
the pace switches to a few units every turn while the price is still
above the base.
"""
...
# Dawn-to-dawn milk price. A drop means the book is being sold by someone
# else; a rise means the town is still drawing inventory down.
...
def _sell_qty(
    ...
    # Shared book. A falling dawn price means another farm is selling into
    # the same deficit. Match that pace while the quote is still above base.
    if _TREND["down"] and price >= base:
        return min(have, 3)
    if price < base * 1.15:
        return min(have, 4)
    if day < 17:
        return 0
    if day >= 28:
        return min(have, max(1, (have + 23) // 24))
    production = max(1.0, float(herd) * per_day)
    if drain >= production * 0.7 and day >= 23:
        frac = 1.0
    elif day >= 23:
        frac = 0.35
    else:
        frac = 0.13
    daily = int(round(frac * have))
    if daily <= 0:
        return 0
    per_turn = max(1, (daily + 23) // 24)
    interval = max(1, 24 // max(1, daily))
    if int(hour) % interval != 0:
        return 0
    return min(have, per_turn)
...
def _note_milk(obs: Mapping[str, Any]) -> None:
    ...
    if hour != 0:
        return
    prev = _TREND["dawn"]
    if prev is not None and price < float(prev) - 4:
        _TREND["down"] = True
