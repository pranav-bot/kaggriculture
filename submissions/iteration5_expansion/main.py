"""Iteration 5: surge policy with guarded second-quadrant expansion.

Combines velocity_mill's cash-flow herd controller with rl_fert_mill's
learned Q-table for fertilizer sell timing. The Q-table was trained via
first-visit Monte Carlo and tells the agent when to dump fert at $1 vs
hold for a higher floor.

Additions over velocity_mill:
 - Q-table for daily fertilizer floor decisions: {$1, $20, $40, $60}
 - Fert sell floor is learned, not fixed at $1
"""
...
# Q-table for fertilizer floor decisions (from rl_fert_mill)
FLOORS = (1, 20, 40, 60)
Q: Dict[Tuple[Tuple[int, int], int], float] = {}
_FERT_DAY: Dict[str, int] = {"day": -1, "action": 1}


def _load_q() -> None:
    base_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else os.getcwd()
    path = os.path.join(base_dir, "q_values.json")
    if not os.path.isfile(path):
        return
    with open(path) as handle:
        rows = json.load(handle)
    for stage, band, action, value in rows:
        Q[(int(stage), int(band)), int(action)] = float(value)


def _fert_state(day: int, price: float) -> Tuple[int, int]:
    if day < 10:
        stage = 0
    elif day < 22:
        stage = 1
...
def _fert_prior(state: Tuple[int, int], action: int) -> float:
    _stage, band = state
    if band == 0:
...
def _choose_fert_floor(day: int, price: float) -> int:
    if _FERT_DAY["day"] == day:
        return int(FLOORS[_FERT_DAY["action"]])
    state = _fert_state(day, price)
    best = max(range(4), key=lambda a: _fert_prior(state, a) + Q.get((state, a), 0.0))
    _FERT_DAY["day"] = day
    _FERT_DAY["action"] = best
    return int(FLOORS[best])
...
def _sell_qty(
    ...
    if item == "FERTILIZER":
        # Avoid compounding a low quote; terminal pressure still liquidates.
        if price < 40 and shed_total < 90 and day < 28:
            return 0
        return min(have, 4)
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    # Expand only after the opening herd has a solvent feed buffer. The land
    # order is placed before animal purchases so expansion gets a real slot.
    if (
        day >= 6
        and len(quadrants) < 2
        and money >= 5_500
        and wheat_have >= live + 8
        and len(orders) < 9
    ):
