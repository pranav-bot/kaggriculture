"""care_mill labor with a milk sell path fit by scipy.

The herd, wheat, and hands are unchanged. Milk sales follow the schedule
from scripts/quant_milk_opt.py: hold through day 16, sell about 13% of
the shed per day on days 17–22, then clear the shed on days 23–29 when
shop drain can absorb the herd. A thin town sells only about 35% per day
late, so the sqrt premium is not walked through in one batch.
"""
