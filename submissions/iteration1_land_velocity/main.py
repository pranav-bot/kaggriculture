"""Iteration 3: solvent land expansion with a gated melon catalyst.
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    # Land has the highest modeled marginal return when unlocked before day 8.
    if len(quadrants) == 1 and day >= 6 and money >= 4200 and wheat_have >= live + 8 and len(orders) < 9:
    ...
    if day == 0 and int(seeds.get("MELON", 0)) < 4 and money >= 320 + OPERATING_RESERVE and len(orders) < 10:
        orders.append(["BUY_SEED", "MELON", 4 - int(seeds.get("MELON", 0))])
        money -= (4 - int(seeds.get("MELON", 0))) * 80
...
def _units(
    ...
    for index, position in enumerate(positions):
        ...
        wheat_planted = scan["crops"].get("WHEAT", 0)
        crop = "MELON" if wheat_planted >= WHEAT_PLOTS and int(seeds.get("MELON", 0)) > 0 else "WHEAT"
        crop_limit = 4 if crop == "MELON" else WHEAT_PLOTS
        if day < 2 and int(seeds.get(crop, 0)) - planted > 0 and scan["crops"].get(crop, 0) + planted < crop_limit:
            target = take_nearest(pos, list(scan["empties"]))
            if target is not None:
                planted += 1
