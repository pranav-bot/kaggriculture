"""surplus_wheat_strawberry — guarded wheat-to-strawberry crop rotation.

This variant preserves the compound policy's feed-safe opening, but replaces
only surplus harvested wheat tiles with strawberries.  Feed is reserved from
live-animal demand, and seed purchases are intentionally not visible to the
unit planner until the next observation.
"""
...
MELON_SEED_COST = 80
STRAWBERRY_SEEDS = 2
STRAWBERRY_SEED_COST = 100
FEED_RESERVE_DAYS = 2
FEED_RESERVE_BUFFER = 2
Pos = Tuple[int, int]

_LOCK: Dict[str, Optional[str]] = {"animal": None}
_ROTATION: Dict[str, set[Pos]] = {"harvested_wheat": set()}
...
def _sell_qty(
    ...
    need_cash: bool, is_endgame: bool, wheat_reserve: int = 0,
) -> int:
    ...
    if item == "WHEAT":
        ...
        if is_endgame:
            return max(0, have - wheat_reserve)
        return max(0, have - wheat_reserve) if shed_total >= 90 else 0
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    wheat_reserve = max(
        4, live * FEED_RESERVE_DAYS + FEED_RESERVE_BUFFER,
    )
    ...
    if day >= 3 and _scores(demand).get(animal, 0) > 0 and owned < target and len(orders) < 9:
    ...
    # Buy rotation seed once, but never rely on this order being available to
    # the unit planner during the same turn.
    strawberry_owned = int(seeds.get("STRAWBERRY", 0))
    if (
        day == 0
        and strawberry_owned < STRAWBERRY_SEEDS
        and money >= OPERATING_RESERVE + 300
        and len(orders) < 10
    ):
        qty = STRAWBERRY_SEEDS - strawberry_owned
        orders.append(["BUY_SEED", "STRAWBERRY", qty])
        money -= qty * STRAWBERRY_SEED_COST
...
def _units(
    ...
    fetchers = 0
    planted = {"WHEAT": 0, "MELON": 0, "STRAWBERRY": 0}
    money = float(farm.get("money", 0))
    ...
    empty_positions = set(scan["empties"])
    # A rotation target is valid only while the original wheat tile has
    # actually been harvested and remains empty.  This prevents arbitrary
    # expansion into unused land or reusing a tile occupied by a new crop.
    _ROTATION["harvested_wheat"].intersection_update(empty_positions)
    ...
    for index, position in enumerate(positions):
        ...
        if day >= 2:
            ripe = [
                ...
                if str(tile.get("crop")) in ("WHEAT", "MELON", "STRAWBERRY")
                and int(tile.get("yield_units", 0)) > 0
            ]
            target = take_nearest(pos, ripe)
            if target is not None:
                tile = next(tile for p, tile in plants if p == target)
                if str(tile.get("crop")) == "WHEAT" and pos == target:
                    _ROTATION["harvested_wheat"].add(target)
                actions.append(["HARVEST"] if pos == target else _move(pos, target))
                continue
        ...
        # Rotate only tiles recorded as harvested wheat.  The seed count is
        # snapshotted from the observation, so a BUY_SEED order above cannot
        # create a same-turn planting assumption.
        strawberry_planted = (
            scan["crops"].get("STRAWBERRY", 0) + planted["STRAWBERRY"]
        )
        strawberry_available = max(
            0, int(seeds.get("STRAWBERRY", 0)) - planted["STRAWBERRY"]
        )
        rotation_targets = [
            p for p in _ROTATION["harvested_wheat"] if p in empty_positions
        ]
        if (
            strawberry_available > 0
            and strawberry_planted < STRAWBERRY_SEEDS
            and rotation_targets
            and int(shed_stock.get("WHEAT", 0))
            + sum(int((inv2 or {}).get("WHEAT", 0)) for inv2 in inventories)
            >= max(4, sum(scan["counts"].values()) * FEED_RESERVE_DAYS + FEED_RESERVE_BUFFER)
        ):
            target = take_nearest(pos, rotation_targets)
            if target is not None:
                planted["STRAWBERRY"] += 1
                _ROTATION["harvested_wheat"].discard(target)
                actions.append(
                    ["PLANT", "STRAWBERRY"] if pos == target else _move(pos, target)
                )
                continue
...
def agent(obs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if day == 0 and int(obs.get("hour", 0)) == 0:
        _LOCK["animal"] = None
        _ROTATION["harvested_wheat"].clear()
