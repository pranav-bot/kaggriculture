"""Stage policy: prebuild pastures, then sell product only while it buys animals.

Days 0–2 hire and build pastures. No feed crop, so labor is not watering
wheat when the first animals arrive. From day 3 one shop-matched species.
Milk, wool, and eggs are sold while the herd is still short of cash, and
held once the herd is paid for. Nothing of those is sold below base.
"""
...
def _sell_qty(
    ...
    # Below base, further sales walk the curve to $1. Stop.
    if price < base:
        return 0
    # While the next animal is unaffordable, turn product into cash the same
    # day. Once the herd is paid for, hold so the town can lift the price.
    if fund or need_cash:
        return min(have, 3 if price >= base * 1.25 else 2)
    if day < 26 and shed_total < 82:
        return 0
    pace = 6 if day >= 26 or shed_total >= 90 else 3
...
def _market(obs: Mapping[str, Any], scan: Mapping[str, Any], animal: str, demand: Mapping[str, float]) -> List[List[Any]]:
    ...
    wheat_floor = 36 if live == 0 else live * 2 + 4
    ...
    if day >= 3 and _scores(demand).get(animal, 0) > 0 and owned < target and len(orders) < 10:
        ...
        if buy > 0 and wheat_have >= live + buy + 1 and money >= spec["cost"] * buy + 40:
...
def _units(
    ...
    for index, position in enumerate(positions):
        ...
        if day < 3:
            need_struct = EARLY_PASTURES
        else:
            need_struct = min(_target(day, len(quadrants)), _drain_cap(animal, demand, len(quadrants)))
        if day < 26 and have_struct < max(need_struct, owned if day >= 3 else need_struct):
