"""rl_fert_mill plus pastures on days 0–2, without cutting the wheat crop.

Eight wheat plots still go in first. Then workers build up to ten
pastures so the fertilizer-funded cows have a stall the day they are
bought. Fertilizer still uses the learned price floor.
"""
...
# action 0 floor $1, 1 floor $20, 2 floor $40, 3 floor $60
FLOORS = (1, 20, 40, 60)
MODE = "greedy"
EPSILON = 0.0
TRACE: List[Tuple[Tuple[int, int], int]] = []
...
def _state(day: int, price: float) -> Tuple[int, int]:
    if day < 10:
        stage = 0
    elif day < 22:
        stage = 1
...
def _prior(state: Tuple[int, int], action: int) -> float:
    """Scipy sat near $18. Below $20 empty the shed."""
    _stage, band = state
    if band == 0:
...
def _choose(day: int, price: float) -> int:
    ...
    best = max(range(4), key=lambda action: _prior(state, action) + Q.get((state, action), 0.0))
...
def _sell_qty(
    ...
    if item == "FERTILIZER":
        floor = FLOORS[_choose(day, price)]
        if price < floor:
...
def _units(
    ...
    planted = 0
    if day < 3:
        build_budget = max(0, EARLY_PASTURES - (
            scan["counts"][animal] + len(scan["empty"][spec["structure"]])
        ))
    else:
        build_budget = 99
    money = float(farm.get("money", 0))
    ...
    for index, position in enumerate(positions):
        ...
        if build_budget > 0 and day < 26 and have_struct < max(need_struct, owned if day >= 3 else need_struct):
            target = take_nearest(pos, list(scan["empties"]))
            if target is not None:
                build_budget -= 1
