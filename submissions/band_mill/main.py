"""care_mill labor with a price-band milk cap from differential evolution.

Once milk is quoted at or above $210, sell about 15 units a day, spread
across the hours. Below $210, hold, unless the last three days still have
a quote at or above the $160 base. The fit is in scripts/quant_milk_opt.py.
"""
...
def _sell_qty(
    ...
    # $210 is the threshold the evolution search kept returning on a deep
    # town, with or without another seller. 15 units a day is about one
    # cared herd and less than a deep drain, so the deficit can still grow.
    if day >= 27:
        daily = min(have, 24)
    elif price >= 210:
        daily = 15
        ...
        return 0
    sold_this_hour = ((int(hour) + 1) * daily) // 24 - (int(hour) * daily) // 24
    return min(have, max(0, sold_this_hour))
