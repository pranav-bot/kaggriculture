"""care_mill labor with a milk inventory floor that depends on the quote.

Dawn logs showed the town refilling a monopoly book while sales ran, so a
fixed 15-unit cap left milk unsold at $250. This agent keeps only 8 units
while the quote is at least $240, keeps 20 while it is at least $210, and
holds if the quote falls below $210 but stays above the $160 base. The
last two days clear anything still above the base.
"""
