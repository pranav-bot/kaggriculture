"""Stage policy from the matched-shop cap experiment.

Eight wheat plots, hold sales at or above base, slow herd ramp. The cap
is 18 only when shop drain can absorb about 16 cared animals. Otherwise
it stops at 12, or at 8 when the drain is thin. Filling to 18 on a thin
town walks the price back to base and does not pay.
"""
...
def _matched(animal: str, demand: Mapping[str, float]) -> float:
    spec = SPECIES[animal]
    shop = max(0.0, float(demand.get(spec["product"], 0.0)) - 1.0)
    if shop <= 0 or spec["per_day"] <= 0:
        return 0.0
    return shop / spec["per_day"]


def _drain_cap(animal: str, demand: Mapping[str, float], quadrants: int = 1) -> int:
    """Tiers from research/11. 18 only when the open shops can take it."""
    del quadrants
    matched = _matched(animal, demand)
    if matched <= 0:
        return 0
    if matched >= 16:
        return 18
